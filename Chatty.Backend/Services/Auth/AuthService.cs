    ...
    ILogger logger) : IAuthService
{
    ...
    public async Task<Result<bool>> LogoutAsync(string token, CancellationToken ct = default)
    {
        try
        {
            if (!RevokedTokens.TryAdd(token, DateTime.UtcNow))
            {
                return Result<bool>.Failure(Error.Internal("Failed to logout user"));
            }
    ...
    private static bool IsTokenRevoked(string token) => RevokedTokens.TryGetValue(token, out var revokedAt);
