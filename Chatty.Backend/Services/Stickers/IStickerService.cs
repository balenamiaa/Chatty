...
{
    Task<Result<StickerPackDto>> CreatePackAsync(Guid userId, CreateStickerPackRequest request, CancellationToken ct = default);
    Task<Result<StickerDto>> AddStickerAsync(string packId, Stream content, string name, string? description = null, string[]? tags = null, CancellationToken ct = default);
    ...
    Task<Result<IReadOnlyList<StickerPackDto>>> GetAvailablePacksAsync(Guid? serverId = null, CancellationToken ct = default);
