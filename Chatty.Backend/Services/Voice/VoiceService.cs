    ...
    : IVoiceService
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            var participant = await context.CallParticipants
        ...
        {
            logger.LogError(ex, "Failed to update video state for user {UserId} in call {CallId}",
                userId, callId);
        ...
        CancellationToken ct = default)
    {
        ...
        {
            logger.LogError(ex, "Failed to get participants for call {CallId}", callId);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            await eventBus.PublishAsync(new CallStartedEvent(callDto), ct);

            return Result<CallDto>.Success(callDto);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to initiate call");
            return Result<CallDto>.Failure(Error.Internal("Failed to initiate call"));
        }
    }

    public Task<Result<bool>> JoinCallAsync(Guid callId, Guid userId, bool withVideo = false,
    ...
    public Task<Result<bool>>
        MuteParticipantAsync(Guid callId, Guid userId, bool muted, CancellationToken ct = default) =>
        throw new NotImplementedException();

    public Task<Result<bool>> UpdateCallStatusAsync(Guid callId, UpdateCallRequest request,
        CancellationToken ct = default) => throw new NotImplementedException();
        ...
        CancellationToken ct = default)
    {
        ...
        {
            logger.LogError(ex, "Failed to send signaling message in call {CallId} from user {UserId}", callId, userId);
